...
# MOSFET models (level-1, simplified)
...
# ---------------------------------------------------------------------------
# Build the ring: node n{i} is the input of stage i and the output of
# stage i-1.  The last stage drives n1, closing the loop.
# ---------------------------------------------------------------------------
nodes = [f"n{i + 1}" for i in range(N_STAGES)]   # ["n1", "n2", "n3"]

for i in range(N_STAGES):
    inp = nodes[i]
    out = nodes[(i + 1) % N_STAGES]   # wraps around: stage 3 drives n1
...
# ---------------------------------------------------------------------------
# Kick pulse on n1 to break symmetry (1 ns high, then low forever).
# A 1 GΩ series resistor makes it nearly invisible in steady state.
# ---------------------------------------------------------------------------
c.add(VoltageSource("kick", "kick_src", "0",
                    waveform="PULSE(3.3 0 0 1p 1p 1n 1000)"))
...
# ---------------------------------------------------------------------------
# Transient simulation: 10 ns window, 10 ps step
# ---------------------------------------------------------------------------
print("\n=== Simulation output ===")
try:
    analysis = Transient(
